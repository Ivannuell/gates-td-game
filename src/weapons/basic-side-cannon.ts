...
export class BasicSideCannon {
  ...
  update(ts, dt) {
    ...
    this.enemy.forEach(e => {
      e.group.getChildren().forEach(enemy => {
        enemyGroup.push(enemy)
      })
    })
    
    const firstEnemy = this.findNearest(this.gameObject, enemyGroup);
    ...
    this.gameObject.scene.physics.moveTo(bullet, nextX, nextY, 400);

    this.gameObject.setRotation(
      Phaser.Math.Angle.BetweenPoints(this.gameObject, firstEnemy) + Math.PI / 2
    );
  ...
  findNearest(source, targets) {
    ...
    if (targetObjects == null || targetObjects == undefined) {
      return
    }
